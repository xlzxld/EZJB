...
| 主干分支 | **无——项目尚未 `git init`，无 remote**（探测依据：`git rev-parse --is-inside-work-tree` → fatal: not a git repository） | ⚠️ 阻断：主干保护暂无法落地 |
| 已知豁免清单 | `eh-audit.mjs:i18n en 侧键数多于 zh，英文文案含冒号（ON:/failed:/location:/unusable:）被误判为键名` | — |
